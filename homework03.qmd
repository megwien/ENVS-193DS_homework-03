...
Normality = Yes the data is normally distributed, this is shown through the Shapiro test and through the histogram, which shows that the data collected for both variables does not skew right or left.

Based on these two assumption checks the correct test to run would be the Pearson correlation test.
```{r}
#| label: histogram-normality-kelp_elong
# base layer: ggplot
ggplot(kelp, mapping = aes(x = kelp_elong)) + 
...
```{r}
#| label: histogram-normality-temp_c
# base layer: ggplot
ggplot(kelp, mapping = aes(x = temp_c)) + 
  # first layer: histogram
  geom_histogram() + 
  # removing gridlines
  theme_classic()
...
Plot 2. Scatterplot depicting relationship between time of day and commute length in minutes. Each green data point shows a single data point representing a commute length. The plot shows that as the time increases, the length of the commute decreases, showing a negative relationship between time of day and commute length in minutes.
...
### d.
I would recommend that on the first graph specifically they find a way to show the underlying data in a nondistracting way. I think that the simple approach is good, but without seeing the data points it is less convincing of the point they are trying to make. The bar graphs show enough information that if anything was added it would be too cluttered but the line graph does not have too much information currently and would benefit from adding individual data points for each sea. 
